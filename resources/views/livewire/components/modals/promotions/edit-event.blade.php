<x-modal.card title="Edit the event" max-width="4xl" wire:model.defer="editEvent" x-on:close="$wire.reload()">
    @csrf
    @php($id = Str::random(4))
    @if ($photo)
        <label>Preview</label>
        <div class="flex justify-center bg-gray-200 rounded-2xl mb-4">
            <x-image.editor alias="EditEvent" image-to-edit="{{ asset($this->saveAndGetImage()) }}" class="max-w-full" />
        </div>
    @elseif($imageLink)
        <div class="flex justify-center bg-gray-200 rounded-2xl mb-4">
    ...
    @endif
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2 mb-6">
        <div>
            <x-input label="Name" placeholder="Event title" wire:model.debounce="event.title" />
        </div>
        <div>
            <x-textarea label="Description" placeholder="Event description" wire:model.defer="event.description" />
        </div>
    </div>

    @isset($event)
        <livewire:components.modals.promotions.add-elements :event="$event" wire:key="{{ time() . Str::random(4) }}" />
    @endisset
