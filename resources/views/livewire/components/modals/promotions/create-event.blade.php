<x-modal.card title="Create a new event" max-width="4xl" wire:model.defer="newEvent" x-on:close="$wire.reload()">
    @csrf
    @php($id = Str::random(4))
    @if ($photo)
        <label>Preview</label>
        <div class="flex justify-center bg-gray-200 rounded-2xl mb-4">
            <x-image.editor alias="CreateEvent" image-to-edit="{{ asset($this->saveAndGetImage()) }}" class="max-w-full" />
        </div>
    @elseif($imageLink)
        <div class="flex justify-center bg-gray-200 rounded-2xl mb-4">
