<div class="group relative p-4 border-r border-b border-gray-200 sm:p-6" >
    ...
    <div class="pt-10 pb-4 text-center">
        <h3 class="text-sm font-medium text-gray-900">
            @if($addToElements)
                <a class="cursor-pointer" onclick="Livewire.emit('addDishToEventUp', '{{ $dish->uuid }}')">
                    <span aria-hidden="true" class="absolute inset-0"></span>
                    {{ $dish->title }}
                </a>
            @else
                <a onclick="Livewire.emit('editDishUuid', '{{ $dish->uuid }}'); $openModal('editDish')" class="cursor-pointer">
                    <span aria-hidden="true" class="absolute inset-0"></span>
                    {{ $dish->title }}
                </a>
            @endif
        ...
        @endif
        <x-input.maskable mask="###########" id="amount_{{ $dish->uuid }}" class="mt-4" wire:model="amount" wire:change="saveChanges" />
